...
Game::Game(unsigned numPlayers) : numPlayers(numPlayers), initialHandSize(7), guesser(0), cardsNotInPlay(0)
{
	...
	*/// Was getting vector crashing is numPlayers was 10+ reworked to avoid issue.
	deck.shuffleDeck();
...
void Game::guess(unsigned playerGuessing)
{
	...
	while (guessRank < RANK_ACE || guessRank > RANK_KING)
	{
		while (validTest == 0){
			...
			std::vector< Card > temp = players[playerGuessing].cardsOfRank(static_cast<CARD_RANK>(guessRank));
			if (temp.size() > 0){
				validTest = 1;  //Added a test to make sure card you are naming is actually in your hand ie cant guess 2 if dont have no twos fool
			}
			else{
				std::cout << "You must choose a card in your hand!\n\n\n";
				std::cin.clear();
				std::cin.ignore();
			}
